...
public:
	...
	{
		this->data = new ull_t[this->count];//reinterpret_cast<ull_t*>(calloc(this->count, TYPESIZE_IN_BYTES));
	...
	BitSet(ull_t nBits, StackAllocator& allocator) : nBits(nBits), usingAllocator(true)
	{
		...
		data = reinterpret_cast<ull_t*>(memset(allocator.allocate(count * TYPESIZE_IN_BYTES), 0, count * TYPESIZE_IN_BYTES));
	}
